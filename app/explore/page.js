...
function ChiliIcon({ className = "" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} aria-hidden="true">
      <path
        d="M16.7 6.1c1.1 3.2-.7 7.8-4 10.6-2.3 2-5.2 2.6-6.8 1.7-.7-.4-.7-1.2 0-1.6 2.5-1.5 4.1-3.6 5.2-6.1 1-2.3 2.8-4.2 5-4.7l.6.1Z"
        fill="currentColor"
        fillOpacity="0.13"
        stroke="currentColor"
        strokeWidth="1.75"
  ...
  const renderNoPhotoMetadata = () => (
    <div className={`pointer-events-none absolute z-20 flex items-end ${isRestaurant ? "inset-x-3 bottom-[4.45rem]" : "inset-x-2 bottom-[3.05rem] min-h-7"}`}>
      {isRestaurant ? (
        ...
        <div className="flex max-h-[3.45rem] flex-wrap items-end gap-1 overflow-hidden pb-0.5">
