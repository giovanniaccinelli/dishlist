...
function getSystemDishlistIconShellStyle(id) {
  ...
  return {
    ...
    boxShadow: `inset 0 0 16px rgba(${color},0.16), 0 0 14px rgba(${color},0.12)`,
  };
}
                  ...
                  return (
                    <div className="relative h-40 w-full overflow-hidden bg-black">
                      ...
                      {isRestaurantPreview ? (
                        <>
                          {restaurantName ? (
                            <div className="pointer-events-none absolute bottom-[5rem] left-3 right-3 z-20 max-w-[calc(100%-1.5rem)] truncate rounded-full border border-[#E64646]/38 bg-[#2A1010]/88 px-3.5 py-1.5 text-[12px] font-black leading-none text-[#FFD4D0] shadow-[0_0_18px_rgba(230,70,70,0.18)]">
                              {restaurantName}
                            </div>
                          ) : null}
                        </>
                ...
                {visibleProfileDishlists.map((dishlist, index) => {
                  ...
                  return (
                    ...
                    <button
                      ...
                      className={`relative w-full rounded-[1.5rem] p-3 text-left ${isTagDishlist ? `aspect-square border-2 shadow-[0_12px_28px_rgba(0,0,0,0.08)] ${getTagDishlistCardClass(dishlist, darkMode)}` : neonDishlistStyle ? "profile-system-dishlist-card text-white" : darkMode ? "border border-white/10 bg-[#151515] shadow-[0_12px_28px_rgba(0,0,0,0.08)]" : "border border-black/10 bg-white shadow-[0_12px_28px_rgba(0,0,0,0.08)]"}`}
                    ...
                    >
                      <div className="mb-2 flex items-center justify-between gap-2">
                        ...
                        {!isTagDishlist ? (
                          <span
                            className={`relative shrink-0 ${neonIconShellStyle ? "grid h-8 w-8 place-items-center rounded-full" : ""}`}
