...
export default function CommentsModal({
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 z-[90] flex items-end justify-center bg-black/42 px-3 pb-3 pt-16 sm:items-center sm:p-4"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
        >
          <motion.div
            className="flex max-h-[calc(100dvh-5rem)] w-full max-w-md min-h-0 flex-col overflow-hidden rounded-[2rem] border border-[#E3CFA7] bg-[linear-gradient(180deg,#FFF9F1_0%,#FFF4E5_52%,#F7F6F1_100%)] shadow-[0_28px_80px_rgba(0,0,0,0.24)]"
            ...
            <div className="shrink-0 border-t border-black/8 bg-white/52 px-4 py-3">
              ...
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="min-h-12 max-h-24 flex-1 resize-none rounded-[1.25rem] border border-black/10 bg-white px-4 py-3 text-sm text-black shadow-[0_8px_20px_rgba(0,0,0,0.04)] focus:outline-none focus:ring-2 focus:ring-[#2B74B8]/20"
                  rows={1}
                  disabled={disabled}
                />
                <button
                  type="button"
                  onClick={onSubmit}
                  className={`flex h-12 shrink-0 items-center justify-center rounded-[1.1rem] px-4 text-sm font-semibold shadow-[0_10px_24px_rgba(43,116,184,0.22)] ${
                    disabled || !hasText
