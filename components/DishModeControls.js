...
export function DishModeFilterButton({ value = DISH_MODE_ALL, onClick, onSelect, className = "" }) {
  ...
  const restaurantIconSizeClass = useLargeSize ? "h-[2.05rem] w-[2.05rem]" : "h-[1.58rem] w-[1.58rem]";
  ...
  return (
    <div className={`relative flex items-center gap-2 ${className}`} aria-label="Filter dish mode">
      ...
      >
        <RestaurantForkKnifeIcon className={restaurantIconSizeClass} strokeWidth={1.85} />
      </button>
      <AnimatePresence>
        {flashMessage ? (
          <motion.div
