...
function getRestaurantTagIconMarkup(tag = "") {
  ...
  return iconMarkup.replace(`style="display:block;color:${iconColor};filter:drop-shadow(0 1px 1px rgba(0,0,0,0.28))" `, `style="display:block;color:#111111" `);
}
...
function getRestaurantMarkerIcon(markerTone = "default", { showDefaultSymbol = true } = {}) {
  ...
  const strokeColor = selected ? "#F2C94C" : markerTone === "own" ? "#2BD36B" : markerTone === "followed" ? "#E4B43F" : "#3A1A1A";
  const fillColor = selected ? "#F2C94C" : markerTone === "own" ? "#12351F" : markerTone === "followed" ? "#332B10" : "#E64646";
  return {
...
function createRestaurantPinOverlay({
  ...
  overlay.onAdd = function onAdd() {
    ...
    const fillColor = tagTheme?.fill || (markerTone === "selected" ? "#F2C94C" : markerTone === "own" ? "#12351F" : markerTone === "followed" ? "#332B10" : "#E64646");
    ...
    <div className={embedded ? `h-full w-full overflow-hidden ${className}` : `restaurant-accent-border min-h-[22rem] overflow-hidden rounded-[2rem] border-2 bg-[#F4EFE6] shadow-[0_24px_50px_rgba(0,0,0,0.10)] ${className}`}>
      <div className="relative h-full min-h-0 overflow-hidden rounded-[inherit]">
          ...
          <div className="absolute left-3 right-3 top-[4.2rem] z-[11] flex items-center justify-between gap-2">
            ...
            <div className="no-accent-border inline-flex h-8 items-center gap-0.5 rounded-full bg-[#050505]/90 p-0.5 text-white shadow-[0_8px_22px_rgba(0,0,0,0.28)] backdrop-blur-md">
              <button
                ...
                style={restaurantFilter === "all"
                  ? {
                      background: "linear-gradient(135deg,#12351F 0%,#0B2114 100%)",
                      boxShadow: "inset 0 0 0 1px rgba(43,211,107,0.55), 0 0 16px rgba(43,211,107,0.20)",
                      color: "#A7F3BE",
                      WebkitTextFillColor: "#A7F3BE",
                    }
              ...
              <button
                ...
                style={restaurantFilter === "following"
                  ? {
                      background: "linear-gradient(135deg,#12351F 0%,#0B2114 100%)",
                      boxShadow: "inset 0 0 0 1px rgba(43,211,107,0.55), 0 0 16px rgba(43,211,107,0.20)",
                      color: "#A7F3BE",
        ...
        <AnimatePresence>
          {categoryFilterOpen ? (
            <motion.div
              className="absolute inset-0 z-[40] bg-[#070907] p-3"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              <motion.div
                className="absolute left-3 right-3 top-[6.55rem] max-h-[min(24rem,calc(100%-8rem))] overflow-y-auto rounded-[1.25rem] border border-[#2BD36B]/22 bg-[#0D0D0D] p-3 text-white shadow-[0_18px_50px_rgba(0,0,0,0.36),0_0_28px_rgba(43,211,107,0.08)]"
              ...
              >
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setCategoryFilterOpen(false)}
                    className="flex h-8 w-8 items-center justify-center rounded-full bg-white/8 text-white/65"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`flex min-h-11 items-center gap-2 rounded-[0.95rem] border px-3 text-left text-sm font-bold ${
                      !restaurantCategoryFilter ? "border-[#2BD36B] bg-[#0D2117] text-[#A7F3BE] shadow-[inset_0_0_18px_rgba(43,211,107,0.18),0_0_18px_rgba(43,211,107,0.10)]" : "border-white/10 bg-white/6 text-white/72"
                    }`}
                  >
                  ...
                  {RESTAURANT_CATEGORY_OPTIONS.map((category) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex min-h-11 items-center gap-2 rounded-[0.95rem] border px-3 text-left text-sm font-bold ${
                          active ? "text-white" : category.chip || "border-white/10 bg-white/8 text-white"
                        }`}
                        style={active ? {
                          borderColor: category.fill,
                          background: `linear-gradient(135deg,${category.stroke} 0%,#101010 100%)`,
                          boxShadow: `inset 0 0 18px ${category.fill}44, 0 0 18px ${category.fill}22`,
                        } : undefined}
                      >
