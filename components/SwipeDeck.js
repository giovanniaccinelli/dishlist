          ...
          {hasBottomActionRow ? (
            <div className="absolute left-4 right-4 z-30 flex items-center justify-between" style={{ bottom: actionBottom }}>
              <button
                ...
                className="add-action-btn action-btn-white-ring no-accent-border h-14 w-14 shrink-0"
                aria-label="Secondary action"
                disabled={disabled}
              >
                {resolvedSecondaryActionLabel === "Edit" ? <Pencil size={18} strokeWidth={2.1} /> : resolvedSecondaryActionLabel}
              ...
              <button
                ...
                className={`add-action-btn action-btn-white-ring no-accent-border h-14 w-14 shrink-0 ${String(actionClassName || "").includes("text-[#2BD36B]") ? "text-[#2BD36B]" : "text-[36px]"}`}
                aria-label="Action"
                disabled={disabled}
              >
                {actionLabel === "+" ? <Plus size={26} strokeWidth={2.1} /> : actionLabel}
